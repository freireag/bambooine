...
describe AccountsController do
  describe "route generation" do
    it "should route the new accounts action correctly" do
      route_for(:controller => 'accounts', :action => 'new').should == "/login"
    end
    it "should route the create accounts correctly" do
      route_for(:controller => 'accounts', :action => 'create').should == "/account"
    end
    it "should route the destroy accounts action correctly" do
      route_for(:controller => 'accounts', :action => 'destroy').should == "/logout"
    end
  end
  
  describe "route recognition" do
    it "should generate params from GET /login correctly" do
      params_from(:get, '/login').should == {:controller => 'accounts', :action => 'new'}
    end
    it "should generate params from POST /account correctly" do
      params_from(:post, '/account').should == {:controller => 'accounts', :action => 'create'}
    end
    it "should generate params from DELETE /account correctly" do
      params_from(:delete, '/logout').should == {:controller => 'accounts', :action => 'destroy'}
    end
  end
  ...
  describe "named routing" do
    ...
    it "should route account_path() correctly" do
      account_path().should == "/account"
    end
    it "should route new_account_path() correctly" do
